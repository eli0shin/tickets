...
Every supplied criterion must match. Repeated status criteria search the union of those statuses; all other repeated criteria use AND semantics. Blocker and unblocked criteria apply only to non-completed tickets, so they always exclude tickets in `done`, including when `done` is an explicit status criterion. Search is not free-text search and has no generic filtering language.
