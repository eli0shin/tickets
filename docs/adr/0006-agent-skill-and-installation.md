...
1. **Resolve and inspect.** Use the Tickets CLI to discover the selected project and use `list`, `search`, and `show` to inspect work. Treat the first `show` output line as the resolved ticket's absolute path and the remaining output as its complete document. Consult `tickets --help` or command-specific help when syntax is needed. This step is complete when the exact project and tickets being operated on are known.
